<template>
    <main class="four-card-feature-section">
        <div class="container">
            <header>
                <h1>Reliable, efficient delivery</h1>
                <h2>Powered by Technology</h2>
                <p>Our Artificial Intelligence powered tools use millions of project data points to ensure that your project is successful</p>
            </header>
            <div class="cards">
                <div v-for="card in cards" :key="card" class="card" :class="card.Theme">
                    <div class="card-body">
                        <h3>{{card.Title}}</h3>
                        <div class="content">{{card.Content}}</div>
                        <img :src="'data:'+card.Image.mime+';base64,'+card.Image.data" :alt="card.Image.alt" />
                    </div>
                </div>
            </div>
        </div>
    </main>
</template>

<script lang="ts">
import '../assets/scss/style.scss';
import { defineComponent } from "@vue/runtime-core";

export default defineComponent({
    name: "FourCardFeatureSection",
    data() {
        const cards: Array<any> = [
            {
                Title: "Supervisor",
                Content: "Monitors activity to identify project roadblocks",
                Image: {
                    "alt": "Eyeglass",
                    "mime": "image/svg+xml",
                    "data": ""
                },
                Theme: "cyan"
            },
            {
                Title: "Team Builder",
                Content: "Scans our talent network to create the optimal team for your project",
                Image: {
                    "alt": "House",
                    "mime": "image/svg+xml",
                    "data": ""
                },
                Theme: "red"
            },
            {
                Title: "Karma",
                Content: "Regularly evaluates our talent to ensure quality",
                Image: {
                    "alt": "Lightbulb",
                    "mime": "image/svg+xml",
                    "data": ""
                },
                Theme: "orange"
            },
            {
                Title: "Calculator",
                Content: "Uses data from past projects to provide better delivery estimates",
                Image: {
                    "alt": "Monitor",
                    "mime": "image/svg+xml",
                    "data": ""
                },
                Theme: "blue"
            }
        ]
        return {
            cards
        }
    }
})
</script>